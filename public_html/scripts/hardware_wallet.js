			...
			return new Promise(function(resolve, reject) {
				...
				return self.obtainExclusiveLock().then(function() {
					...
					if(self.isConnected() === true) {
						...
						// Return requesting verifying the root public key on the hardware wallet
						return self.send(HardwareWallet.REQUEST_CLASS, HardwareWallet.REQUEST_VERIFY_ROOT_PUBLIC_KEY_INSTRUCTION, HardwareWallet.NO_PARAMETER, HardwareWallet.NO_PARAMETER, Buffer.from((new BigNumber(HardwareWallet.ACCOUNT)).toBytes(BigNumber.LITTLE_ENDIAN, Common.BYTES_IN_A_UINT32))).then(function(response) {
							...
							// Otherwise
								...
								// Reject error
								reject(Message.createText(Language.getDefaultTranslation('Verifying the root public key on that %1$x hardware wallet failed.'), [productName]) + " " + Message.createText(Language.getDefaultTranslation('Make sure that the correct app is open on the hardware wallet.')));
							}
						...
						// Catch errors
							...
							if(typeof error === "object" && error !== null && "statusCode" in error === true && error["statusCode"] === HardwareWallet.USER_REJECTED_RESPONSE_STATUS) {
								...
								// Reject error
								reject(Message.createText(Language.getDefaultTranslation('Verifying the root public key on that %1$x hardware wallet failed.'), [productName]));
							}
